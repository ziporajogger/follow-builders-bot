...
- 所有 URL 原样保留，并且直接输出裸 URL（不要用 markdown 链接语法 [文字](URL) 包起来），链接显示文字就用 URL 本身，不要用中文短语（如"推特链接"、"读他的文章"）或英文原文当链接文字。
...
- 一个 builder 一天发好几条是正常的，不限制条数：所有有价值的推文都保留，每条用一句话概括并附上它自己的链接，逐条列出。只跳过没价值的（见下一条）。不要给推文加"原推文 / 原推文（补充）"这类自造标签（它们只是不同的推文，是并列关系，不是原文和补充的关系），不要先贴英文原文再补中文。
